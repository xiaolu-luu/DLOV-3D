...
def main(ctx: DictConfig):
    ...
    device = "cpu" 
    device = get_free_gpu(min_mem=6000) if torch.cuda.is_available() else device
    ...
    masks = InstanceMasks3D(ctx.data.masks.masks_path)#masks.masks.shape = (237360,148)
    ...
    pointcloud = PointCloud(ctx.data.point_cloud_path)# pointcloud.points.shape = (237360,3)
