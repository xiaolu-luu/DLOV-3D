...
class QuerySimilarityComputation():
    ...
    def get_per_point_colors_for_similarity(self, 
        ...
        for mask_idx, mask in enumerate(masks[::-1, :]): #mask_idx=0~147 mask.shape = (237360,)
            ...
            x=openmask3d_per_mask_scores_rescaled[len(masks)-mask_idx-1]
            print(x)
            co = plt.cm.jet(x)[:3]
            print(co)
            new_colors[mask>0.5, :] = co
...
def main():
    ...
    path_openmask3d_features = "output-test3/m_openmask3d_features.npy" #"output/scene_example_openmask3d_features.npy"
    ...
    pred_masks = np.asarray(torch.load(path_pred_masks)).T # (num_instances, num_points) shape:(148, 237360) array 0:no 1:yes
    ...
    # Set the query text
    # --------------------------------
    query_text = 'pillow'#'faucet'#'chair'#"refrigerator\sink\window" # change the query text here
