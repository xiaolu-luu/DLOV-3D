...
class VLMapBuilder:
    ...
    def create_mobile_base_map(self):
        ...
        self.base_poses = np.loadtxt(self.pose_path) #base_poses.shape = (441, 7)
        ...
        cv_map = np.zeros((gs, gs, 3), dtype=np.uint8) # cv_map.shape = (1000, 1000, 3)
        height_map = -100 * np.ones((gs, gs), dtype=np.float32) #shape:(1000, 1000)
        ...
        for frame_i, (rgb_path, depth_path, base_posevec) in enumerate(pbar):
            ...
            rgb = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB) # rgb.shape = (720, 1080, 3)
            depth = load_depth_npy(depth_path) # depth.shape = (720, 1080)
            # edge_width = 250
            ...
            )# pix_feats.shape = (1, 512, 347, 520)
            pix_feats_intr = get_sim_cam_mat(pix_feats.shape[2], pix_feats.shape[3])#shape:(3, 3)

            # backproject depth point cloud
            pc = self._backproject_depth(depth, calib_mat, depth_sample_rate, min_depth=0.1, max_depth=6)#shape:(3, 7726)
            ...
            pc_transform = tf @ self.base_transform @ self.base2cam_tf #shape:(4, 4)
            pc_global = transform_pc(pc, pc_transform)  # (3, N) shape:(3, 7726)
    ...
    def _init_map(self, camera_height: float, cs: float, gs: int, map_path: Path) -> Tuple:
        ...
        vh = int((camera_height / cs)+130)# 1.5/0.05 = 30
        grid_feat = np.zeros((gs * gs, self.clip_feat_dim), dtype=np.float32)# grid_feat.shape = (1000000, 512)
        grid_pos = np.zeros((gs * gs, 3), dtype=np.int32) #grid_pos.shape = (1000000, 3)
        occupied_ids = -1 * np.ones((gs, gs, vh), dtype=np.int32) #occupied_ids.shape = (1000, 1000, 30)
        weight = np.zeros((gs * gs), dtype=np.float32) #weight.shape = (1000000,)
        grid_rgb = np.zeros((gs * gs, 3), dtype=np.uint8)#grid_rgb.shape = (1000000, 3)
        mapped_iter_set = set()#集合是一种无序且不重复的数据结构，可以用于存储唯一的元素。在这里，mapped_iter_set用于存储已经映射的迭代器。
        ...
        if os.path.exists(map_path):
            ...
            max_id = grid_feat.shape[0] # max_id = 67929
