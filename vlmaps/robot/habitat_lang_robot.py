...
class HabitatLanguageRobot(LangRobot):
    ...
    def setup_scene(self, scene_id: int):
        ...
        self.scene_name = vlmaps_data_dir.name#[0] #.split("_")'5LpN3gDmAk7'

        self._setup_sim(self.scene_name)

        self.setup_map(vlmaps_data_dir)

        cropped_obst_map = self.map.get_obstacle_cropped() #cropped_obst_map = (422, 362)
    ...
    def _setup_sim(self, scene_name: str):
        """
        重写,加载ithor仿真器'FloorPlan212'
        Setup Habitat simulator, load habitat scene and relevant mesh data
        """
        if self.sim is not None:
            self.sim.close()
        # self.test_scene = os.path.join(self.test_scene_dir, scene_name, scene_name + ".glb")#'/home/ztl/deeplearning/vlmaps/v1/tasks/mp3d_habitat/mp3d/5LpN3gDmAk7/5LpN3gDmAk7.glb'
        # self.sim_setting = {
        #     "scene": self.test_scene,
        #     **self.config["params"]["sim_setting"],
        # }
        controller = Controller(
            agentMode="locobot",#default
            visibilityDistance=3.5,
            scene=scene_name , #"FloorPlan212"

            # step sizes
            gridSize=0.01,
            snapToGrid=False,
            rotateStepDegrees=self.turn_angle,
        ...
        if self.sim is None or scene_name == self.last_scene_name:
            ...
            self.sim = controller
            self.event = controller.reset(scene=scene_name)
        else:
    ...
    def set_agent_state(self, tf: np.ndarray):
        ...
        rot = r.as_euler('zxy',degrees=True)
        rot_quat = r.as_quat()
        agent_rot_y = rot[2]
        # self.sim.get_agent(0).set_state(agent_state)
        event = self.sim.step(
            action="Teleport",
            position=dict(x=agent_x,y=agent_y,z=agent_z),
            rotation=dict(x=0,y=agent_rot_y ,z=0),
            horizon=0,
            #standing=True,
        )
        self._set_nav_curr_pose(event)
    ...
    def execute_actions(
        ...
        for action_i, action in enumerate(actions_list):
            ...
            if action=='move_forward':
                ...
                position_i = agent_state['position']
                position.append(position_i)
            if len(position)>5:
                position_a =position[-1]
                position_b =position[-5]
                position = []
                dis = (position_a['x']-position_b['x'])**2+(position_a['z']-position_b['z'])**2
                # print("=================",dis,"=================")
                if dis<0.025:
                    break
            if vis:
                # self.display_obs(waitkey=True)
                self.display_curr_pos_on_map(map,event)
            if poses_list is None:
                continue

            row, col, angle = self._get_full_map_pose(event)
            if vis:
    ...
    def _execute_action(self, action: str):
        if action=='turn_right':
            self.sim.step(action="RotateRight",degrees=self.turn_angle)
        elif action == 'turn_left':
            self.sim.step(action="RotateLeft",degrees=self.turn_angle)
        else :
            self.sim.step(action="MoveAhead",moveMagnitude=self.forward_dist)
    ...
    def _set_nav_curr_pose(self,event):
        """
    ...
    def _get_full_map_pose(self,event) -> Tuple[float, float, float]:
        # agent_state = self.sim.get_agent(0).get_state()
        agent_state = event.metadata['agent']
        hab_tf = agent_state2tf(agent_state)
        self.vlmaps_dataloader.from_habitat_tf(hab_tf)
    ...
    def display_curr_pos_on_map(self, map: np.ndarray,event):
        row, col, angle = self._get_full_map_pose(event)
...
def main(config: DictConfig) -> None:
    ...
    obs = robot.sim.last_event.frame
    rgb = obs#["color_sensor"]
    bgr = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR)
    cv2.imshow("scr rgb", bgr)
    cv2.waitKey()

    tar_hab_tf = cvt_pose_vec2tf(robot.vlmaps_dataloader.base_poses[98])
