...
class LangRobot:
    ...
    def move_to_object(self, name: str):
        ...
        self.move_to(pos)
        # pos_agent = self.sim.last
        event = self.sim.step(
            action="Done"
        )
        self._set_nav_curr_pose(event)
        pos_agent = list(self.curr_pos_on_map)
        dis = (pos[0]-pos_agent[0])**2+(pos[1]-pos_agent[1])**2
        for i in range(3):
            if dis >2000:
                self.move_to(pos)
                self._set_nav_curr_pose(event)
                ...
                self.recorded_actions_list.append('stop')
                break
            if i ==2:
                self.recorded_actions_list.append('stop')
        
        # agent_state = event.metadata['agent']
        # pos_agent = agent_state["posation"]
