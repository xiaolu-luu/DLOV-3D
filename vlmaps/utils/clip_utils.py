...
def get_lseg_score(
    ...
    """
    landmarks_other = landmarks#38个类别
    if add_other and landmarks_other[-1] != "other":
        landmarks_other = landmarks + ["other"]

    if use_multiple_templates:
        mul_tmp = multiple_templates.copy()#63个模板
        multi_temp_landmarks_other = [x.format(lm) for lm in landmarks_other for x in mul_tmp]#39*63=2457个模板
        text_feats = get_text_feats(multi_temp_landmarks_other, clip_model, clip_feat_dim)#text_feats.shape = (2457, 512)

        # average the features
        if avg_mode == 0:
            text_feats = text_feats.reshape((-1, len(mul_tmp), text_feats.shape[-1]))#text_feats.shape = (39, 63, 512)
            text_feats = np.mean(text_feats, axis=1)#text_feats.shape = (39, 512)

        map_feats = lseg_map.reshape((-1, lseg_map.shape[-1])) # map_feats.shape = (198640, 512)

        scores_list = map_feats @ text_feats.T # scores_list.shape = (198640, 39)
